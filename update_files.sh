...
if [ $# -eq 1 ]; then
    if [ $1 -eq 25 ]; then
        ...
        rsync -ram -e 'sshpass -p upmRobotica ssh' --include '*/' --include '*.avi' --exclude '*' robotica@192.168.1.25:myDir/resources/videos ./resources
    elif [ $1 -eq 26 ]; then
        ...
        rsync -ram -e 'sshpass -p upmRobotica ssh' --include '*/' --include '*.avi' --exclude '*' robotica@192.168.1.26:myDir/resources/videos ./resources    
    else
        usage
    fi
else
